function [ACC_PRA, ACC_Base, train_outputs, test_outputs] = PRA_PL_LEAF(X_train,y_train,X_test,y_test, k,ker,C1,C2,epsi,par,tol)
%PRA_PL_LEAF implements the PRA framework presented in [1] with PL-LEAF as the base learner
%
%    Syntax
%
%       [ACC_PRA, ACC_Base, train_outputs, test_outputs] = PRA_PL_LEAF(X_train,y_train,X_test,y_test, k,ker,C1,C2,epsi,par,tol)
%
%    Description
%
%       PRA_PL_LEAF takes,
...
%  [1] J.-Y. Liu, J.-P. Sun, Y.-H. Zhao, B.-B. Jia. Augmenting Graph-Based Partial Label Learning with Predictive Representations. Electronics, 2026, 15(18), Article 4210. https://doi.org/10.3390/electronics15184210 
    ...
    %include the PL-LEAF package, avaiable at: https://palm.seu.edu.cn/zhangml/files/PL-LEAF.rar
    addpath('PL_LEAF');%Note that we have made some necessary modifications to the software package.
    ...
    if nargin < 8  || isempty(C2)
        C2 = 1; %Penalty parameter
    end
    if nargin < 7  || isempty(C1)
    ...
    [train_outputs1,test_outputs1,ACC_Base] = PL_LEAF(X_train,X_train,y_train,X_test,y_test,k,ker,C1,C2,epsi,par,tol);
    ...
    [train_outputs, test_outputs, ACC_PRA] = PL_LEAF(X_train_t,X_train_m,y_train,X_test_t,y_test,k,ker,C1,C2,epsi,par,tol);
    rmpath('PL_LEAF');
end
